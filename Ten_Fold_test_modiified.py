...
class MGPred(nn.Module):
    def __init__(self, u_one_embedding, u_two_embedding, u_three_embedding, i_one_embedding, i_two_embedding, i_three_embedding, embed_dim, N = 30000, droprate = 0.5, beta_ema = 0.999):
        ...
        # Embeddings for users and items. embeddings are low-dimensional, dense vector representations of items/inputs
        ...
        self.N = N
        self.droprate = droprate # 50% nodes are randomly dropped at each batch/iteration
        self.beta_ema = beta_ema

        # linear layers (nn.Linear(input_feature_dim,output_feat_dim)) for processing the embeddings.
        # For each of the drug and side-effect embeddings (one, two, three), there are two linear layers (one_layer1, one_layer2, etc.).
        ...
        # Combined layer for drug and side effect embeddings
        self.con_layer1 = nn.Linear(self.embed_dim * 3, self.embed_dim)
        self.con_layer2 = nn.Linear(self.embed_dim, 1)

        # Batch normalization layers are applied after each linear layer to normalize the activations.
        # Batch normalization normalizes the input activations by subtracting the mean and dividing by the standard deviation.
        # This helps in stabilizing and accelerating the training of neural networks.
        ...
        # In neural networks, EMA is often used for parameter averaging.
        # EMA is often used as a form of model averaging or ensemble technique.
        #  It can improve the generalization performance of a model by using a more stable set of parameters.
        # EMA can act as a form of regularization, smoothing out the noise in the training process and preventing overfitting.
        if beta_ema > 0.:
    ...
    def forward(self, nodes_u, nodes_i):
        # The user/drug and item/SE node indices are passed through embedding layers (u_one_embed, u_two_embed, etc.)
        # to obtain the corresponding embeddings for users (u_one_embed, u_two_embed, etc.) and items (i_one_embed, i_two_embed, etc.).
        ...
        # The embeddings are concatenated for each level
        ...
        # The concatenated embeddings go through linear layers (one_layer1, one_layer2, etc.) followed by ReLU activation functions (F.relu).
        # Dropout is applied after each activation function to introduce regularization during training (F.dropout).
        # Batch normalization (one_vi, two_vi,..) is applied after each linear layer using the batch normalization layers
        ...
        #The results from the three levels (ui_one, ui_two, ui_three) are concatenated
        # and passed through another linear layer (con_layer1). Again, ReLU activation and dropout are applied.
        ...
        #The final output is obtained by passing the result through the last linear layer (con_layer2).
        # The scores are then squeezed to remove any singleton dimensions. If the tensor has dimensions with size 1, they will be removed.
        scores = self.con_layer2(x)
        return scores.squeeze()

    # It iterates through the layers of the model (self.layers) and accumulates the regularization terms from each layer.
    #  Regularization introduces a penalty term to the training process,
    #  discouraging the model from becoming too complex or fitting the noise in the training data to avoid overfitting
    ...
    # The update_ema method increments the EMA step count and updates the EMA parameters.
    # This is typically done during training to maintain a moving average of the model parameters.
    def update_ema(self):
        self.steps_ema += 1
        for p, avg_p in zip(self.parameters(), self.avg_param):
            avg_p.mul_(self.beta_ema).add_((1 - self.beta_ema) * p.data)

    # The load_ema_params method loads the EMA parameters into the model's current parameters.
    # This is useful during evaluation or inference to use the averaged parameters.
    def load_ema_params(self):
        for p, avg_p in zip(self.parameters(), self.avg_param):
            p.data.copy_(avg_p / (1 - self.beta_ema ** self.steps_ema))

    # The load_params method loads parameters into the model from a given set of parameters (params).
    # The load_ema_params method is a specific case of loading EMA parameters, and load_params is a more general method for loading any parameters.
    def load_params(self, params):
        for p, avg_p in zip(self.parameters(), params):
            p.data.copy_(avg_p)

    # The get_params method creates a deep copy of the current model parameters. This is used to retrieve the parameters for saving or analysis.
    def get_params(self):
        params = deepcopy(list(p.data for p in self.parameters()))
        return params

    # The loss method computes the total loss,
    # which includes the standard loss computed by the model (self.forward) and the regularization term (self.regularization).
...
def train_test(data_train, data_test, data_neg, fold, args):
    # processes data for training and testing a recommendation system.
    # It involve tasks such as creating embeddings, forming datasets, and generating negative samples for training
    ...
    # Creating DataLoader instances for training, testing, and negative samples data
    ...
    # --- Model initialization (MGPred) -----
    # u2e1, u2e2, and u2e3 are user embeddings corresponding to different interactions.
    # i2e1, i2e2, and i2e3 are item embeddings corresponding to different interactions.
    # u_adj is the user adjacency matrix.
    # drug part
    ...
    u_embed_cmp1 = encoder(embed_dim, u_agg_embed_cmp1, cuda=device) # It takes the aggregated user embeddings (u_agg_embed_cmp1, u_agg_embed_cmp2, u_agg_embed_cmp3) and converts them into final user embeddings.
    ...
    i_embed_cmp1 = encoder(embed_dim, i_agg_embed_cmp1, cuda=device, is_user_part=False) # It takes the aggregated item embeddings (i_agg_embed_cmp1, i_agg_embed_cmp2, i_agg_embed_cmp3) and converts them into final item embeddings.
    ...
    # Model with processed user and item embeddings
    ...
    # Calculate rmse and mae for each epoch and if rmse doesn't got minimized for more than 30 epochs, early stopping
    for epoch in range(1, args.epochs + 1):
        ...
        # If the performance on the test set does not improve for a certain number of epochs (endure_count > 30),
        # the training is stopped to prevent overfitting.
        if endure_count > 30:
            break
